...
# )
partial_patterns = [
                            url(r'^(?P<template>.*)$', manager_views.angular_template,
                                name=ANGULAR_TEMPLATE_URL_NAME),
                            # ... more partials ...,
]
...
urlpatterns = [
    url(r'^$', manager_views.home,
        name=MANAGER_HOME_URL_NAME),
    (r'^rest/', include(rest_api.urls)),
    # url(r'^tiles/', include(tiles_urls_patterns)),
    url(r'^upload/$', manager_views.upload, name=NEW_URL_NAME),
    url(r'^publish/$', manager_views.publish,
        name=PUBLISH_URL_NAME),
    url(r'^save_new/$', manager_views.save_new,
        name=SAVE_NEW_LAYER_URL_NAME),
    url(r'^arcgis/rest/', include(geo_services_rest_patterns)),
    url(r'^angular/templates/', include(partial_patterns,
                                        namespace=APP_NAME + '_angular_partials')),
]
